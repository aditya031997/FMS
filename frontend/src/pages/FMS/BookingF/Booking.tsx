...
const Booking = () => {
    ...
    const [borderCharges, setBorderCharges] = useState(0);
    ...
    const saveUser = async (e: any) => {
        ...
        try {
            if (params.booking_id && !addInvoiceType) {
                ...
                console.log(driverDocumentFile, passportDocumentFile, idcardDocumentFile, truckDocumentFile);
                if (driverDocumentFile || passportDocumentFile || idcardDocumentFile || truckDocumentFile || data) {
                    ...
                    try {
                        const addBooking = await axios.post(`${config.API_BASE_URL}/bookings`, formData);
                        console.log(addBooking.data, 'Add successful');
                        // showMessage('Booking has been saved successfully.');
                    } catch (error: any) {
    ...
    useEffect(() => {
        if (filterrouteData?.borders && filterrouteData.borders.length > 0) {
            const initialCharges = {};
            filterrouteData.borders.forEach((border: any) => {
                initialCharges[border?.borderName] = border.charges;
            });
                                                                    ...
                                                                    ? filterrouteData?.borders?.map((i: any) => (
                                                                          <tr key={i.borderName}>
                                                                              ...
                                                                              <td className="border p-4">
                                                                                  ...
                                                                                  <input
                                                                                      type="text"
                                                                                      value={borderCharges[i?.borderName] || ''}
                                                                                      onChange={(e) =>
                                                                                          setBorderCharges((prevCharges: any) => ({
                                                                                              ...prevCharges,
                                                                                              [i.borderName]: e.target.value,
                                                                                          }))
                                                                                      }
                                                                                  />
                                                                                  {/* ) : (
